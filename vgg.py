...
def get_feature(internel_layer, layers, filters, batch_norm = False, conv_func=mx.sym.Convolution, **kwargs):
    for i, num in enumerate(layers):
        for j in range(num):
            internel_layer = conv_func(data = internel_layer, kernel=(3, 3), pad=(1, 1), num_filter=filters[i], name="conv%s_%s" %(i + 1, j + 1))
            if batch_norm:
...
def get_symbol(num_classes, num_layers=11, batch_norm=False, dtype='float32', conv_func=mx.sym.Convolution, **kwargs):
    """
    Parameters
    ----------
    ...
    feature = get_feature(data, layers, filters, batch_norm, conv_func)
